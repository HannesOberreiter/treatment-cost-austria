...
# Our Libraries
VLIBS <- c("tidyverse", "patchwork", "here", "readxl", "bookdown", "BlandAltmanLeh", "boot", "ggsignif", "lme4")
...
ggplot(size_gsm_plot,
  ...
  geom_text(aes(x = 6, y = 10.3, label=paste("mean =", round(mean(DATA$costs),2))), color = "gray") + 
    geom_text(
      aes(
        x=c(1:6), y = 0, 
  ...
  scale_y_continuous(
    limits = c(0,max(size_gsm_plot$upper)*1.1),
    breaks = c(0:round(max(size_gsm_plot$upper)*1.1))
      ) +
  geom_signif(
    data=costs_size_pairwise,
    aes(xmin=name, xmax=compare, annotations="", y_position = 12),
    textsize = 3, vjust = -0.2, color = "gray", manual=TRUE)
...
p
```
```{r boot-median, include=T, warning = F, fig.cap="Boostrap Median"}
p <- fBootPlot(boot_median, "Median")
p
```
