...
(ref:efficient-economic) Efficiency and economical plot for treatment method combinations with at least *n* = 30 as sum over the three survey years. Letters corresponding to different treatment methods, see Table \@ref(tab:ee-table). Vertical axis the mean expense for each treatment combination with standard error (SE). Horizontal axis the mean observed winter colony loss rates for each treatment combination with 95% confidence interval (CI). The blue dashed line indicate mean total expense (horizontal) and total observed winter colony loss rates (vertical) per year. Categorizing of treatment combinations based on location in relation to the blue dashed lines (Fig.&nbsp;\@ref(fig:efficient-economic-mm)). The four categories are colour coded as seen in the plot legend.

```{r efficient-economic, include=T, fig.cap="(ref:efficient-economic)"}
include_custom("output/figs/efficient-economic.png")
```

(ref:ee-table) Treatment combinations with at least *n*=30 as sum over the three survey years. Ordered by number of participants applying the combinations. 'Efficient' and 'Economical' are colour coded based on low (teal) and high (orange), categorized based to total loss rates and total mean expenses (Fig.&nbsp;\@ref(fig:efficient-economic)). Abbrevations: SP/SU/WI = Spring/Summer/Winter || Fa-LT/ST = Formic acid - long/short term || Ox-sub/pure/mix = Oxalic acid - sublimation, pure or mixture || Biot = Another biotechnical methods.
...
```{r ee-table-small, include=T}
cn <- c(" ", "Treatment", "Year", "\\textit{n}", "Median", "Mean", "SD", "Mean", "Lower CI", "Upper CI", "Efficient", "Economical")
cell_colors_price <- c("high-expense" = "cred!80", "low-expense" = "cgreen!80")
cell_colors_loss <- c("high-loss" = "cred!80", "low-loss" = "cgreen!80")

tab <- kbl(
    r_treatment$tab_small %>%
        select(-quantile_efficient, -quantile_economical) %>%
        mutate(
            c_short_od = str_replace_all(c_short_od, "&", " \\\\& "),
            across(letter:c_short_od, ~ ifelse(lag(as.character(.x), default = "0") == .x, "", .x)),
