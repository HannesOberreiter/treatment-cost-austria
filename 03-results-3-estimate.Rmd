...
The self calculated estimated expenses, based on online research of product prices, central tendency values are similar to the survey responses in each year. The estimate mean is on average `r round(mean(as.numeric(r_estimate$table$mean[1:3]) - as.numeric(r_estimate$table$mean[4:6])), 2)` Euro higher than the survey mean and the estimate median is on average `r round(mean(as.numeric(r_estimate$table$median[1:3]) - as.numeric(r_estimate$table$median[4:6])), 2)` Euro higher than the survey median (Table \@ref(tab:table-estimates)).
...
The comparison between estimate and reported survey expenses not separated by year but for the different treatment method combinations (excluding drone brood removal) shows that the total overestimation is not true for all treatment methods, as some are also underestimated. This comparison can be visualized as differences plot (Fig. \@ref(fig:estimates-bland)). Typically this is done as a statistical method of agreement between methods [@bland1986]. On average the difference between treatment method combinations with at least 15 participants is close to zero (`r round(r_estimate$ba$mean.diffs, 2)` Euro), indicating no over- or underestimation (Fig. \@ref(fig:estimates-bland), yellow line). Only one combination is outside the 95% confidence interval of the upper and lower two times standard deviations from the mean difference. This combination of summer and winter oxalic acid trickling of ready made mixture ('SU-OX-Mix & Wi-Ox-Mix') was underestimated on average by 15 Euros, although the high difference is mainly driven by extrem outliers in this group (*n* = 22, mean expenses survey = 19 Euro, median expenses survey = 9 Euro). Overestimation of the estimated expenses are shown for treatment combinations which include biotechnical methods excluding drone brood removal or hypothermia in summer ('SU-Biot') and oxalic acid sublimation in winter ('WI-Ox'). The three most common treatment combinations, formic acid long term in summer ('SU-Fa-LT') and winter oxalic treatment ('WI-Ox-sub', 'WI-Ox-pure', 'WI-Ox-mix'), show similar values for mean estimate and mean reported survey expenses (Fig. \@ref(fig:estimates-bland)). For the top 11 treatment method combinations and the comparison of the mean and median expenses of the estimate and as reported from the participants see Table \@ref(tab:common-comb).
...
(ref:estimated-bland) Difference plot between survey and estimated expenses of treatment method combinations with at least 15 beekeepers using it, over the three years of survey. Negative values on the vertical axis are underestimates and positive values are overestimates of the estimated expenses. Lines indicating mean difference (yellow) and lower and upper two times standard deviation (blue) each with 95% confidence interval. Point size is the sample count of each treatment method combination. Treatment combination inside the two times standard deviations CI range and the top 3 combinations based on number of participants are colour coded and labelled. SP/SU/WI = Spring/Summer/Winter || AS-LT/ST = Formic acid - long/short term || Ox-sub/pure/mix = Oxalic acid - sublimation, pure or mixture || Biot = Another biotechnical methods.
