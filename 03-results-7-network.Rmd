...
(ref:treatment-network) Network graph of the treatment methods over all three survey years (*n*=`r nrow(dfClean) %>% ft()`). Node sizes represents count how often a method was reported. Node colours are the seasons 'Spring' (April-May; Green), 'Summer' (June-October; Black) and 'Winter' (November-March; Blue). Edge connections colour and width are the reported count of each combination. Categories with less than 30 reported answers were not included. Abbreviations: Drone = Drone brood removal || Fa-LT/ST = Formic acid - long/short term || Ox-sub/pure/mix = Oxalic acid - sublimation, pure or mixture || Biot = Another biotechnical method || Thy = Thymol || Hyp = Hyperthermia || syn Met = synthetic Methods.

(ref:treatment-heat) Absolute numbers of reported treatments in each month over the three survey years (*n*=`r nrow(dfClean) %>% ft()`). Colour coded as heatmap with darker colours representing more answers. No answers in white color.
