...
In addition the same extrapolation was calculated with the geometric mean from the natural log ($\ln$). For this calculation the single zero value in the dataset was replaced with EUR&nbsp;0.01. The geometric mean is less influenced by outliers and typically used as point estimate for a log-normal distribution. The results are always lower than the arithmetic mean and the difference in the extrapolation is on average EUR&nbsp;1,258,377.
