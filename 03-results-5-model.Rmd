...
tab <- kable(
    r_model$fitted %>% mutate(model = c("Base", "Base + Operational", "Base + Operational + Treatment")) %>% select(model, r.squared, adj.r.squared, df:BIC),
    booktabs = TRUE,
    col.names = cn,
    digits = 1,
    align = c("l", rep("r", 9)),
