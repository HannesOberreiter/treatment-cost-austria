...
To evaluate how good the selected questions can be used for prediction of the treatment expenses multiple regression model with treatment expenses per colony as dependent variable was fitted. The model creation and evaluation was done with the tidymodels package [@tidymodels2020]. 

(Insert log10 Distribution results)

Data preprocessing included removing the single zero expenses answer. Normalization  $log_10$ transform of expenses and number of colonies wintered

So, these two features are in very different ranges. When we do further analysis, like multivariate linear regression, for example, the attributed income will intrinsically influence the result more due to its larger value. But this doesn’t necessarily mean it is more important as a predictor. So we normalize the data to bring all the variables to the same range.

 only participants which did not answers the afore mentioned operational factors or did answer "Uncertain" were removed, to simplify the model parameters (Tab. \@ref(tab:factor-removed)). Treatment methods were binary transformed, if the participants did use it in any of the month in the survey to 1 (True) otherwise if not applied to 0 (False). Model fitting was done with 3/4 of the original dataset (n = `r nrow(r_model$train_data)`), first model evaluation was done with standard parameters (eg. $R^2$, AIC, BIC) and out of sample evaluation was done with `r nrow(r_model$test_data)` rows to account for possible overfitting. Importance plot of variables based on @vip2020.
